...
### 20260524r0 — 2026-05-24 — P3 tech-debt sweep (R12)
- **DataUpdateCoordinator (D15)** introduced (`coordinator.py`). Sensor platform now refreshes through a single hub-wide `eps` request instead of one `ep` per entity — request count drops from N to 1 per cycle. binary_sensor and cover remain on per-entity polling for now (tracked as follow-up).
- **Diagnostics (D16)**: download an entry/device diagnostic payload from the HA UI (Settings → Devices & Services → integration → menu → Download diagnostics). Includes hub_info, devtype counts, full device payload (token/sign redacted).
- **Repair issues (D17)**: when `cfg:getver` returns an unexpected shape, an HA Issue (`hub_version_unknown`) is raised and shown in the Repairs panel. Cleared automatically once the next setup succeeds.
- **Async migrate (D14)**: `ConfigFlow.VERSION` bumped to 2 with an `async_migrate_entry` hook that runs the R10 unique_id rewrite via the standard HA path. The legacy in-`async_setup_entry` migration also remains for idempotent safety.
- **Pytest tests (D12)**: new `tests/` directory; 21 tests covering `generate_entity_id`, `_extract_state_changes`, `_migrate_unique_ids`. Run with `pytest tests/`. conftest stubs HA so no HA install required.
- **Translations (D13)**: added `zh-Hant.json` and `zh-Hans.json`; English file gained an `issues` section.
- **Tight exception handling (D11)**: 24 bare `except Exception` reduced to 2 (only listener-isolation dispatchers, annotated with `# noqa: BLE001`).

### 20260523r6 — 2026-05-24 — remote entity_id cross-hub fix (R10-b)
- **Multi-hub completeness.** R10 fixed `unique_id`s on every sub-device platform, but `remote` was the one platform whose `entity_id` did not flow through `generate_entity_id` (it used `remote.<slugify(name)>_<me>` for readability). With two hubs that have identically-named remotes, HA would suffix the second one with `_2`.
- **New pattern:** `remote.<slugify(name)>_<agt>_<me>`. The pre-existing rename block in `remote.py` automatically migrates legacy entity_ids in the registry.
- **Heads-up:** unlike R10, this change DOES rewrite `entity_id`s for existing single-hub users. Any automation / dashboard referencing the old `remote.<name>_<me>` form needs to be updated. (Other platforms were not affected by R10.)
...
├── __init__.py
├── conftest.py
├── test_extract_state_changes.py
├── test_generate_entity_id.py
